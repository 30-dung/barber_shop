...
import 'booking_datetime_screen.dart'; // Navigate to booking date/time screen
...
class _EmployeeSelectionScreenState extends State<EmployeeSelectionScreen> {
  ...
  _loadEmployees() async {
    ...
    try {
      ...
      final employees = await ApiService.getEmployees(storeId: widget.store.id);
      setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Chọn Nhân Viên tại ${widget.store.name}')),
      body:
          _isLoading
      ...
      child: ListTile(
        ...
        subtitle: Column(
          ...
          children: [
            ...
            if (employee.rating != null) // Keep if backend sends rating
              Row(
                children: [
                  Icon(Icons.star, size: 16, color: Colors.amber[700]),
                  const SizedBox(width: 4),
                  Text(
                    '${employee.rating!.toStringAsFixed(1)} sao',
        ...
        onTap: () {
          // Navigate to BookingDateTimeScreen, passing all necessary details
          Navigator.of(context).push(
            MaterialPageRoute(
              builder:
                  (context) => BookingDateTimeScreen(
                    store: widget.store,
                    storeService:
                        widget
                            .storeService, // Pass the entire StoreService object
                    employee: employee,
                  ),
            ),
          );
