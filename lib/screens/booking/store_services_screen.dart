...
class _StoreServicesScreenState extends State<StoreServicesScreen> {
  ...
  _loadStoreServices() async {
    ...
    try {
      final services = await ApiService.getServicesByStoreId(widget.store.id);
      setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Dịch vụ tại ${widget.store.name}')),
      body:
          _isLoading
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Text(
                  '${storeService.price.toStringAsFixed(0)}đ',
